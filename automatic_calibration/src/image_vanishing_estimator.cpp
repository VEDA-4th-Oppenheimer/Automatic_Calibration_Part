...
std::vector<VanishingDirection> estimateVanishingDirections(
  ...
  // Weighted SVD null-space refinement of a seed direction over its support.
  auto refine_axis = [&](const Eigen::Vector3d &seed) -> Eigen::Vector3d {
    std::vector<int> members;
    cluster_support(seed, &members);
    if (members.size() < 2)
    ...
    Eigen::JacobiSVD<Eigen::Matrix3d> svd(scatter, Eigen::ComputeFullU |
                                                       Eigen::ComputeFullV);
    Eigen::Vector3d d = svd.matrixU().col(2);
    ...
    if (mean_ray.norm() > 1e-9 && d.dot(mean_ray) < 0)
      d = -d;
    return d;
  };
